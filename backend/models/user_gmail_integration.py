...
class UserGmailIntegration(Base):
    __tablename__ = "user_gmail_integrations"

    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True, unique=True)
    provider: Mapped[str] = mapped_column(String(32), nullable=False, default=GMAIL_PROVIDER)
    ...
    scope: Mapped[str | None] = mapped_column(String(MAX_SCOPE_LENGTH), nullable=True, default=GMAIL_SCOPES)
    ...
    user: Mapped["User"] = relationship("User", back_populates="gmail_integration")
